    ...
    <SettingsCard title="聚合行为">
      <label className="flex items-center">
        <input
          ...
          onChange={(e) =>
            onSettingChange("aggregateInNewTab", e.target.checked)
          }
        />
    ...
    <SettingsCard title="恢复行为">
      <label className="flex items-center">
        <input
          ...
          onChange={(e) =>
            onSettingChange("restoreInNewWindow", e.target.checked)
          }
        />
    ...
    <SettingsCard title="通知设置">
      <label className="flex items-center">
        <input
          ...
          onChange={(e) =>
            onSettingChange("showNotifications", e.target.checked)
          }
        />
    ...
    <SettingsCard title="分组管理" description="高级分组管理选项。">
      ...
      <label className="flex items-center mb-4">
        <input
          ...
          onChange={(e) =>
            onSettingChange("excludePinnedTabs", e.target.checked)
          }
        />
      ...
      <div className="mb-4">
        ...
        <input
          ...
          onChange={(e) =>
            onSettingChange("maxGroupsToKeep", parseInt(e.target.value))
          }
        />
    ...
    <SettingsCard title="安全设置" description="数据保护和确认选项。">
      <label className="flex items-center mb-4">
        <input
          ...
          onChange={(e) =>
            onSettingChange("confirmBeforeDelete", e.target.checked)
          }
        />
      ...
      {settings.autoBackup && (
        <div>
          ...
          <select
            ...
            onChange={(e) =>
              onSettingChange("backupInterval", parseInt(e.target.value))
            }
          >
...
const DataManagement: React.FC = () => {
  ...
  useEffect(() => {
    chrome.runtime
      .sendMessage({ action: "getStatistics" })
      .then((response: any) => {
        if (response.success) {
    ...
    if (file) {
      ...
      reader.onload = (e) => {
        try {
          ...
          chrome.runtime
            .sendMessage({ action: "importData", data })
            .then((response: any) => {
              if (response.success) {
                alert("导入成功！");
    ...
    <div>
      <SettingsCard title="统计信息">
        ...
        <div className="mt-4 text-sm text-gray-500">
          平均每个分组包含{" "}
          {stats.groupCount > 0
            ? Math.round(stats.tabCount / stats.groupCount)
            : 0}{" "}
          个标签页
        </div>
      </SettingsCard>
